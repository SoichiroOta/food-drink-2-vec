...
if __name__ == '__main__':
    recipe_data = json.load(open('examples/data/suntory_recipe2vec.json'))
    all_ingredients = json.load(open('examples/data/suntory_all_ingredients.json'))
    df = pd.DataFrame(recipe_data).sort_index(axis=1) #.drop(all_ingredients, axis=1)
    tag_df = pd.DataFrame(json.load(open('examples/data/suntory_tag2vec.json'))).sort_index(axis=1)
    ...
    fd2v.model.save('examples/h5_objects/suntory_recipe_ingredient2vec_model2.h5')
